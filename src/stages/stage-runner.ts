...
import { StageName, StageRetryStatus, StageStatus, stageEvent } from '../common/pipeline';
 ...
 * supply their config and their two `@OnEvent` entry points.
 */
export abstract class StageRunner {
  protected readonly logger = new Logger(this.constructor.name);
  protected readonly instanceId = env.instanceId;
  ...
  private async attempt(orderId: string, attempt: number): Promise<void> {
    ...
    const failure = this.rollFailure();
    if (!failure) {
      ...
      this.events.emit(stageEvent(this.config.stage, 'completed'), { orderId, attempt });
      this.logger.log(
    ...
    this.events.emit(stageEvent(this.config.stage, 'failed'), { orderId, attempt, error: failure });
    this.logger.warn(
    ...
    const decision = await this.dataSource.transaction(async (manager) => {
      ...
      if (outcome === RetryDecision.DeadLetter) {
        ...
        await manager.insert(Outbox, {
          ...
          payload: { orderId, stage: this.config.stage, attempt: attempt + 1 },
  ...
  private async simulateWork(): Promise<void> {
    ...
    const delay = Math.round(minDelayMs + this.random() * (maxDelayMs - minDelayMs));
    await new Promise((resolve) => setTimeout(resolve, delay));
  }

  private rollFailure(): string | null {
    return this.random() < this.config.failureRate
      ? `${this.config.stage} dependency unavailable (simulated)`
      : null;
  }
}
