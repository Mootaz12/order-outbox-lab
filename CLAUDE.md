...
3. Stages are not hand-written classes: `stageHandler()` (`src/modules/stages/services/stage-handler.factory.ts`) builds one `StageRunner` subclass per row of `STAGE_CONFIGS` (`consts/stages.constants.ts`), each with `@OnEvent(OrderEvent.Created)` and its own `<stage>.retry`. It must stay one distinct class per stage (decorator metadata is per-prototype); class names are set to `PaymentService` etc. because they're the logger context. Adding a stage = `StageName` + `STAGES` in `src/shared/pipeline.ts` + a `STAGE_CONFIGS` row. All logic lives in `src/modules/stages/services/stage-runner.service.ts`:
   ...
   - `run()` must never reject: eventemitter2 discards handler promises, and an unhandled rejection kills the Node process (default since Node 15).
...
- **Every feature uses the same folder layout:** `<feature>.module.ts` at the root, a `README.md` (purpose, routes/events, file map, invariants — keep it current when behaviour changes), then only the subfolders it needs — `controllers/`, `dtos/` (query/body DTO classes), `services/` (all injectables, incl. health indicators and the stage handler factory), `helpers/` (pure functions), `entities/`, `types/`, `consts/`, `tests/` (`*.spec.ts` for that feature). File suffixes match the folder: `.controller.ts`, `.dto.ts`, `.service.ts`/`.indicator.ts`/`.factory.ts`, `.helper.ts`, `.entity.ts`, `.types.ts`, `.constants.ts`. Don't put a new file at the feature root. Tests construct classes directly with small hand-written in-memory fakes of our own collaborators (fake `EventBus` subclass, fake `DataSource`/manager, fake repositories) — no real DB/Redis, no mocking libraries.
...
- **`src/base/` holds every base others extend** — base entities, base DTOs, base interfaces/types — imported via `@base/*`. Don't put a new base class inside a feature. Today: `BaseEntity`, `BaseQueryDto` (`limit` 1–200 default 50, `order` default `Desc`, defaults in `base.constants.ts`).
- **List endpoints take a query DTO** in the feature's `dtos/` that extends `BaseQueryDto` and adds typed filters (`@IsEnum` on our enums). The global `ValidationPipe({ transform: true, whitelist: true })` in `main.ts` builds the instance with defaults and returns 400 on invalid params; services receive the DTO, never raw strings. A DTO may redeclare `order` to change the default direction (stage history is `Asc`). Queries live in services, not controllers. Sort with the `Order` enum (`src/shared/order.enum.ts`), never `'ASC'`/`'DESC'` literals. DTO specs run the same `ValidationPipe` config; Jest loads `reflect-metadata` via `setupFiles`.
...
- **Entity classes are suffixed `Entity`** (`OrderEntity`, `OutboxEntity`, `OrderStageEventEntity`, `StageRetryEntity`); files stay `<name>.entity.ts`. uuid-keyed entities extend our `BaseEntity` (`src/base/base-entity.ts`: uuid `id`, `created_at`, `updated_at`, soft-delete `deleted_at`) instead of redeclaring them; any raw SQL `UPDATE` on those tables must set `updated_at = now()` (TypeORM only stamps its own writes), and raw reads that should skip soft-deleted rows need `deleted_at IS NULL`; `order_stage_events` (bigserial id) and `stage_retries` (composite key) don't. Import `BaseEntity` from `@base/base-entity`, never from `typeorm` (TypeORM's same-named active-record class would compile without these columns). The file must not be named `*.entity.ts` (entity glob). Validate route params with Nest's built-in pipes (e.g. `ParseUUIDPipe`) rather than hand-written regexes. `POST /orders` is still parsed by hand in `helpers/parse-create-order.helper.ts` into interfaces in `types/orders.types.ts` (not a DTO class, so the global `ValidationPipe` skips it).
- **Column names come from `SnakeNamingStrategy`** (`typeorm-naming-strategies`, set in `database-options.ts`): don't pass `name:` to column decorators. Keep table names explicit in `@Entity('table')` — the strategy would derive them from the `…Entity` class names. The one explicit column name is `@JoinColumn({ name: 'order_id' })` on `OutboxEntity`, which shares the column with `orderId`.
- **Runtime is Node 24 in Docker** (`node:24-alpine`); pnpm comes from corepack via `packageManager`.
